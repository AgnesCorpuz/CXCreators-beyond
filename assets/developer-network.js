...
;define("developer-network/components/generic/img-uploader-container", ["exports", "developer-network/config/environment", "croppr"], function (_exports, _environment, _croppr) {
  "use strict";
  ...
  let croppr = null;
  /**
   * Initiate the croppr library and add the callback functions.
   * This is definitely a non-Ember and 'hacky' way of doing it as the library
   * and this function directly modifies the DOM but it works so...
   * @param {Component} componentRef reference to this Ember Component
   */

  function addCroppr(componentRef) {
    // Slight delay before start.
    setTimeout(() => {
      if (!document.getElementById('croppr')) return; // Canvas elements that will contain the cropped image

      let logo = componentRef.get('newLogo');
      let imgLogo = new Image();
      imgLogo.src = logo;
      let canvasEl = document.createElement('canvas');
      let ctx = canvasEl.getContext('2d'); // Debounce

      let allowAssign = true;
      let debounceTime = 150; // Assign the image to the component variable newLogo

      let assignImage = cropData => {
        if (!allowAssign) return;
        canvasEl.width = cropData.width;
        canvasEl.height = cropData.height;
        ctx.drawImage(imgLogo, cropData.x, cropData.y, cropData.width, cropData.height, 0, 0, cropData.width, cropData.height);
        componentRef.set('newLogo', canvasEl.toDataURL('image/jpeg'));
        allowAssign = false;
        setTimeout(() => allowAssign = true, debounceTime);
      }; // Initiate croppr


      croppr = new _croppr.default('#croppr', {
        aspectRatio: 1,
        minSize: [50, 50, 'px'],
        startSize: [100, 100, '%'],
        onCropMove: function (data) {
          assignImage(croppr.getValue());
        },
        onInitialize: function (instance) {
          assignImage(instance.getValue());
        }
      });
    }, 30);
  }
  /**
   * Reset croppr to the default element
   */


  function resetCroppr() {
    if (!croppr) return;
    croppr.destroy();
  }
...
;define("developer-network/templates/components/global/app-foundry-header", ["exports"], function (_exports) {
  ...
  var _default = Ember.HTMLBars.template({
    "id": "8U3wyA/F",
    "block": "{\"symbols\":[],\"statements\":[[7,\"div\"],[11,\"class\",\"header-container\"],[9],[0,\"\\n    \"],[7,\"div\"],[11,\"class\",\"top-container\"],[9],[0,\"\\n        \"],[7,\"div\"],[11,\"class\",\"logo-container\"],[9],[0,\"\\n            \"],[7,\"img\"],[11,\"src\",\"assets/img/beyond-logo-white.png\"],[11,\"alt\",\"\"],[11,\"class\",\"logo\"],[12,\"onclick\",[29,\"action\",[[24,0,[]],\"navigateHome\"],null]],[9],[10],[0,\"\\n        \"],[10],[0,\"\\n\"],[4,\"if\",[[25,[\"showSearchBar\"]]],null,{\"statements\":[[0,\"                \"],[1,[29,\"filter-profile/search-bar\",null,[[\"location\"],[\"header\"]]],false],[0,\"\\n\"]],\"parameters\":[]},null],[0,\"\\n        \"],[7,\"div\"],[11,\"class\",\"header-right-side\"],[9],[0,\"\\n            \"],[1,[23,\"global/login-widget\"],false],[0,\"\\n        \"],[10],[0,\"\\n    \"],[10],[0,\"\\n    \\n\"],[10],[0,\"\\n\"]],\"hasEval\":false}",
    "meta": {
...
;
          if (!runningTests) {
            require("developer-network/app")["default"].create({"PICTURE_MAX_SIZE":200,"INTL_LANGUAGES":{"en-us":"English","fr":"Français"},"name":"developer-network","version":"0.0.0+7002344f"});
          }
